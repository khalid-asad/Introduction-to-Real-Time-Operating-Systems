...
extern OS_EVENT *SWITCH_A;
extern OS_EVENT *SWITCH_B;
extern OS_EVENT *RED_LED;
extern OS_EVENT *GREEN_LED;
...
int main(void) {
    ...
    alt_up_character_lcd_string(lcd_0, "COE4DS4 Winter16");
...
int init_OS_data_structs(void) {
	...
	SWITCH_A = OSMutexCreate(MUTEX_SWITCH_A, &return_code);
	alt_ucosii_check_return_code(return_code);

	SWITCH_B = OSMutexCreate(MUTEX_SWITCH_B, &return_code);
	alt_ucosii_check_return_code(return_code);

	RED_LED = OSMutexCreate(MUTEX_RED_LED, &return_code);
	alt_ucosii_check_return_code(return_code);

	GREEN_LED = OSMutexCreate(MUTEX_GREEN_LED, &return_code);
	alt_ucosii_check_return_code(return_code);
